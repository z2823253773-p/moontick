...
**后续复核更正：**本记录固定的是首个候选 `18f7cc3` 及其 ZIP。复核发现其包内
CHANGELOG 仍称“只是候选、尚未发布”，若原样发布就会留下新的过时文案。
修复与新的固定包 SHA 见 [0.1.1 独立复核](codex-review-0.1.1.md)；本记录保留
作为原始候选证据，不把旧 ZIP 的 hash 当作最终候选。
