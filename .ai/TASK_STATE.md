...
日期：2026-09-21。T1 已获明确开工授权；当前等待真实 Mooncakes 模块用户名。
...
- 真实 Mooncakes 模块用户名。隔离 moon new 仅生成 username/...；不得猜测或提交。
...
参赛者提供真实 Mooncakes 模块用户名后，Claude Code 从 T1 开始：先写并观察三个
RED 测试，再实现最小 core/ticks/CLI，提交明确 SHA 与证据；Codex 随后独立验收。
